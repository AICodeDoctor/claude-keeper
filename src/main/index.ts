...
import { runPatternDoctor } from './pattern-doctor-runner';
import { WEAK_LIMIT_PATTERNS } from '../core/limit-detector';
...
/**
 * Pattern doctor: once per (command, pattern-set) per app run, scan the
 * installed CLI binary for banner-shaped limit phrasings and warn when one
 * would slip past the active patterns — so a CLI wording change surfaces at
 * session start instead of as a silently missed limit days later. Entirely
 * best-effort and async; never blocks or fails a launch.
 */
const doctorScanned = new Set<string>();
function kickPatternDoctor(
  command: string,
  cwd: string | undefined,
  patterns: RegExp[],
  notify: (message: string) => void,
): void {
  const cacheKey = `${command}|${patterns.length}`;
  if (doctorScanned.has(cacheKey)) return;
  doctorScanned.add(cacheKey);
  const binary = resolveCommand(command, { cwd });
  if (!binary) {
    log.info('doctor: command not resolvable, skipping scan', { command });
    return;
  }
  const weakSources = new Set(WEAK_LIMIT_PATTERNS.map((p) => p.source));
  const strong = patterns.filter((p) => !weakSources.has(p.source));
  const weak = patterns.filter((p) => weakSources.has(p.source));
  void runPatternDoctor(binary, strong, weak)
    .then((report) => {
      if (!report.scanned || report.uncovered.length === 0) return;
      const samples = report.uncovered.slice(0, 3).map((s) => `"${s}"`).join(', ');
      log.warn('doctor: CLI contains limit phrasings the patterns may miss', {
        uncovered: report.uncovered,
      });
      notify(
        `Heads-up: your Claude CLI contains ${report.uncovered.length} limit phrasing(s) the ` +
          `current detection patterns may miss, e.g. ${samples}. Consider adding a custom ` +
          `pattern in Settings.`,
      );
    })
    .catch((err) => {
      log.warn('doctor: unexpected failure', {
...
function wireSession(win: BrowserWindow): void {
  ...
  const buildController = (cfg: SessionStartConfig): SessionController => {
    ...
    // Verify the active patterns against the CLI binary we are about to run
    // (async, cached, best-effort — see kickPatternDoctor).
    kickPatternDoctor(cfg.command || 'claude', cfg.cwd, patterns, (message) => {
      if (!win.isDestroyed()) {
        win.webContents.send(IPC.sessionEvent, toWireEvent({ type: 'notice', message }));
      }
    });
