...
7. Verifies the resume took (retries with backoff if the limit message comes
   back — and if the reply names a clearly later reset, it reschedules to that
   time instead of burning retries), and **Resume now** skips the countdown
   whenever you want. Even a fully exhausted retry budget isn't a dead end:
   after a cool-down the app **self-recovers** back into the wait.
8. Survives **app restart and laptop sleep** mid-wait (state persisted to disk).
9. At session start, a **pattern doctor** scans your installed `claude` binary
   for limit phrasings and warns if a future CLI wording change wouldn't be
   caught by the active detection patterns.
